...
class Delicious_Recipes_REST_Global_Settings_Controller extends Delicious_Recipes_API_Controller {
	...
	public function get_settings( $request ) {
		...
		$global_settings = get_option( 'delicious_recipe_settings' ) ? get_option( 'delicious_recipe_settings' ) : array();
		...
		if ( ! empty( $settings_data ) ) {
			foreach( $settings_data as $key => $setting ) {
				if( ! is_array( $setting ) ) {
					...
					} elseif( 'printLogoImagePreview' === $key ) {
	...
	public function prepare_item_for_response( $request ) {
		...
		$recipe_settings = get_option( 'delicious_recipe_settings', true );
